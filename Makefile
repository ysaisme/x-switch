...
build-web:
	...
	@echo "copying web_dist to cmd/xswitch/web_dist..."
	rm -rf cmd/xswitch/web_dist
	cp -r web_dist cmd/xswitch/web_dist

build:
	$(GO) build -ldflags "$(LDFLAGS)" -o bin/xswitch ./cmd/xswitch

build-all: build-web build
	@echo "built bin/xswitch ($(VERSION), $(shell ls -lh bin/xswitch | awk '{print $$5}'))"

run: build
	./bin/xswitch start

clean:
	rm -rf bin/ web_dist/ cmd/xswitch/web_dist build/ release/
...
install: build
	cp bin/xswitch /usr/local/bin/xswitch
...
release: build-web
	...
	@for platform in $(PLATFORMS); do \
		...
		GOOS=$$GOOS GOARCH=$$GOARCH CGO_ENABLED=0 $(GO) build -ldflags "$(LDFLAGS)" \
			-o release/xswitch-$(VERSION)-$$GOOS-$$GOARCH$$ext ./cmd/xswitch; \
	done
...
app: build-all
	...
	@cp bin/xswitch build/$(APP_NAME).app/Contents/Resources/xswitch
	...
	@chmod +x build/$(APP_NAME).app/Contents/Resources/xswitch
...
dmg: app
	...
	@hdiutil create -volname "X Switch" -srcfolder build/dmg_temp -ov -format UDZO build/$(APP_NAME).dmg
